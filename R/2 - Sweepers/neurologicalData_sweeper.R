# neurologicalData_sweeper.R
# Runs the Hoegn-index design-matrix sweep for the neurological (EEG/signal) blocks.
# Prerequisite: run DIABLO_designMatrix_neurologicalData.R first so that
...
# `design_matrix` must be loaded from DIABLO_designMatrix_neurologicalData.R
design_matrix_ref <- design_matrix

diablo_ref <- block.splsda(
  X      = X,
  Y      = Y,
  ncomp  = 5,        # IMPORTANT: keep consistent with the sweep loop below
  keepX  = keepX,
...
for (i in seq_along(design_grid)) {
  ...
  diablo_i <- block.splsda(
    X      = X,
    Y      = Y,
    ncomp  = 5,        # IMPORTANT: must match ncomp used for diablo_ref above
    keepX  = keepX,
    design = design_i
  )
